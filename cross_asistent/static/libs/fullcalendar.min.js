    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        select: function(info) {
            if ($("#eventModal").hasClass("calendar_update")) {
                ...
                const horaActual = `T${hora.toString().padStart(2, "0")}:${minutos.toString().padStart(2, "0")}`;
                const horaSiguiente = `T${((hora + 1) % 24).toString().padStart(2, "0")}:${minutos.toString().padStart(2, "0")}`;
        
                // Corregir la fecha de fin restando 1 día
                const endDate = new Date(info.end);
                endDate.setDate(endDate.getDate() - 1);
        
                const endStrFix = endDate.toISOString().split("T")[0]; // formato YYYY-MM-DD
        
                $("#eStart").val(`${info.startStr}${horaActual}`).trigger("blur");;
                $("#eEnd").attr("min", `${info.startStr}${horaActual}`);
                $("#eEnd").val(`${endStrFix}${horaSiguiente}`);
            }
        }
